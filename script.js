...
// AI Backend Endpoint
const AI_ENDPOINT = "https://your-backend-url.onrender.com/advice";
...
// AI advisory fetch
async function fetchAIAdvice(city, country, aqi) {
    try {
        const res = await fetch(`${AI_ENDPOINT}?city=${encodeURIComponent(city)}&country=${encodeURIComponent(country)}&aqi=${aqi}`);
        const data = await res.json();
        document.getElementById("ai-advisory-box").innerText = data.advice || "AI advice unavailable";
    } catch (err) {
        console.error("AI fetch error:", err);
        document.getElementById("ai-advisory-box").innerText = "AI advice fetch failed";
    }
}
...
function updateDisplay(name, country, aqi, weather) {
    ...
    processSafety(aqi, name, country);
}

function processSafety(aqi, city = null, country = null) {
    ...
    // AI advisory call
    if(city && country) fetchAIAdvice(city, country, aqi);
}
